...
public class Board : MonoBehaviour
{
    ...
    {
        Vector3 startPos = new Vector2(transform.position.x + col, transform.position.y + 1.2f);
        Vector3 endPos = new Vector2(transform.position.x + col, transform.position.y - ((height - 1) - y));

        float vel = 0.0f;

        token.transform.position = startPos;
        while (Mathf.Approximately(token.transform.position.y, endPos.y) == false)
        {
            vel += -42f * Time.deltaTime;
            token.transform.Translate(new Vector3(0, vel * Time.deltaTime, 0));
            if (token.transform.position.y < endPos.y)
            {
    ...
    void CreateMouseCollider(int col)
    {
        GameObject columnCollider = new GameObject("col" + col.ToString());
        columnCollider.tag = "ColumnCollider";
        columnCollider.transform.parent = transform;
        columnCollider.transform.localPosition = new Vector3(col, -2, 0);
        columnCollider.transform.rotation = Quaternion.identity;
        BoxCollider2D bc = columnCollider.AddComponent<BoxCollider2D>();
        bc.isTrigger = true;
        bc.size = new Vector2(1, height + 1);
    }

    GameObject ActiveToken(bool p1)
    {
    ...
    {
        ActiveToken(p1).transform.position = new Vector2(transform.position.x + col, transform.position.y + 1.2f);
    }

    public void SetTokenToMousePos(bool p1, Vector2 position)
    {
        ActiveToken(p1).transform.position = position;
    }
}
